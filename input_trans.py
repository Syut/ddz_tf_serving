...
import hand_type
from trans_utils import ary2one_hot, str2ary, list2ary
...
def complement(rounds_ary, role=None):
    ...
    while beg + 1 < len(rounds_copy):
        ...
        if next_seat != rounds_copy[beg + 1][0]:
            rounds_copy.insert(beg + 1, (next_seat, empty))
        beg += 1
    if role is not None:
        rounds_copy.pop()
    return rounds_copy
...
def rounds_info(card_list, card_ary, role):
    ...
    ret_round = []
    ret_hand = [[], [], []]
    if len(card_list) == 1 and np.sum(card_list[0][1]) == 20:
        # 地主一轮甩完
        if role == 0:
            ret_round.append(card_ary)
        empty = np.zeros(15, dtype=np.int32)
        ret_round.append(empty)
        ret_hand[0].append(card_list[0][1])
...
def get_before(rounds_ary, role):
    ...
    for i in range(2 - role):
        rounds_copy.insert(i, (role + i + 1, empty))
    each_rounds = []
    for i in range(2):
        each_rounds.append(rounds_copy[i::3])
    for i in range(len(each_rounds[0])):
        if np.sum(each_rounds[1][i][1]) > 0:
            before_label.append(hand_type.ary2label(each_rounds[1][i][1]))
        else:
            before_label.append(hand_type.ary2label(each_rounds[0][i][1]))
    return before_label
